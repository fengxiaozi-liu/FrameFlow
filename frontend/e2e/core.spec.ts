...
const providers = [
  { code: "story-test", name: "故事测试接口", capability: "story", model: "mock-story", base_url: "mock://local", enabled: true, status: "healthy" },
  { code: "image-test", name: "生图测试接口", capability: "image", model: "mock-image", base_url: "mock://local", enabled: true, status: "healthy" },
  { code: "video-test", name: "视频测试接口", capability: "video", model: "mock-video", base_url: "mock://local", enabled: true, status: "healthy" },
  { code: "video-fail", name: "失败测试接口", capability: "video", model: "mock-fail-video", base_url: "mock://local", enabled: true, status: "healthy" },
];
...
async function taskField(request: APIRequestContext, id: string, field: string) {
...
test("home, studio and WebSocket asynchronous task flow", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(page.getByRole("heading", { name: "开始下一段视频创作" })).toBeVisible();
...
test("failed task can be retried from task center", async ({ page, request }) => {
  const created = await request.post("/api/tasks", { data: { kind: "video", provider_code: "video-fail" } });
  ...
  await expect.poll(() => taskField(request, task.id, "retry_count")).toBeGreaterThan(0);
  await expect.poll(() => taskField(request, task.id, "status")).toBe("failed");
});

test("WebSocket disconnect falls back to incremental HTTP polling", async ({ page }) => {
  await page.routeWebSocket(/\/api\/tasks\/.*\/events/, (socket) => socket.close());
  const fallback = page.waitForRequest((request) => request.url().includes("/event-log?after="));
  await page.goto("/studio");
  await page.getByRole("button", { name: "新增分镜" }).click();
  await page.getByRole("button", { name: "检查并生成视频" }).click();
  await fallback;

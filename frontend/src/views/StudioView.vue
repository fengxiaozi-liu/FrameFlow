...
const drawerPreferredWidth = ref(420);
const drawerMaxWidth = ref(Math.max(360, Math.min(800, window.innerWidth - 320)));
const drawerWidthStorageKey = "frameflow-story-drawer-width";
let drawerResizeStart: { x: number; width: number } | null = null;
...
const storyboardCandidateElement = ref<HTMLElement>();
const sceneBoardElement = ref<HTMLElement>();
...
const storyboardDuration = computed(() => draft.value.story.scenes.reduce((total, scene) => total + (scene.duration_seconds || 0), 0));
function materialForScene(sceneID: string, usage: SceneBinding["usage"]) {
  const materialID = draft.value.bindings?.find((binding) => binding.scene_id === sceneID && binding.usage === usage)?.material_id;
  return materials.value.find((item) => item.id === materialID);
}
function scenePreview(sceneID: string) {
  const image = materialForScene(sceneID, "first_frame") || materialForScene(sceneID, "scene_reference");
  return image ? api.mediaUrl(image.url) : "";
}
function sceneStatus(sceneID: string) {
  const current = tasks.items.filter((item) => item.scene_id === sceneID && item.project_id === workspace.projectId && item.draft_id === draft.value.id && item.kind === "video").at(-1);
  if (current?.status === "running") return "生成中";
  if (current?.status === "queued") return "排队中";
  if (current?.status === "failed") return "生成失败";
  if (draft.value.video_versions?.some((version) => version.scene_id === sceneID)) return "已完成";
  return "待生成";
}
...
onUnmounted(() => { unsubscribe(); if (saveTimer) clearTimeout(saveTimer); stopDrawerResize(); window.removeEventListener("resize", clampDrawerToViewport); });
...
function limitDrawerWidth(width: number) {
  return Math.min(Math.max(width, 360), drawerMaxWidth.value);
}
function clampDrawerToViewport() {
  drawerMaxWidth.value = Math.max(360, Math.min(800, window.innerWidth - 320));
  drawerWidth.value = limitDrawerWidth(drawerPreferredWidth.value);
}
onMounted(() => {
  const stored = Number(localStorage.getItem(drawerWidthStorageKey));
  if (Number.isFinite(stored) && stored >= 360) drawerPreferredWidth.value = stored;
  clampDrawerToViewport();
  window.addEventListener("resize", clampDrawerToViewport);
});
function resizeDrawer(event: PointerEvent) {
  if (!drawerResizeStart) return;
  drawerWidth.value = limitDrawerWidth(drawerResizeStart.width + drawerResizeStart.x - event.clientX);
  drawerPreferredWidth.value = drawerWidth.value;
}
function stopDrawerResize() {
  if (!drawerResizeStart) return;
  drawerResizeStart = null;
  document.body.classList.remove("resizing-story-drawer");
  ...
  localStorage.setItem(drawerWidthStorageKey, String(drawerPreferredWidth.value));
}
function startDrawerResize(event: PointerEvent) {
  if (event.button !== 0 || window.innerWidth <= 720) return;
  event.preventDefault();
  drawerResizeStart = { x: event.clientX, width: drawerElement.value?.getBoundingClientRect().width || drawerWidth.value };
  document.body.classList.add("resizing-story-drawer");
...
function resizeDrawerWithKeyboard(event: KeyboardEvent) {
  ...
  event.preventDefault();
  const next = event.key === "Home" ? 360 : event.key === "End" ? 800 : drawerWidth.value + (event.key === "ArrowLeft" ? 32 : -32);
  drawerWidth.value = limitDrawerWidth(next);
  drawerPreferredWidth.value = drawerWidth.value;
  localStorage.setItem(drawerWidthStorageKey, String(drawerPreferredWidth.value));
}
...
function drawerKeydown(event: KeyboardEvent) {
  ...
  const focusable = [...drawerElement.value.querySelectorAll<HTMLElement>('button:not([disabled]), textarea, select:not([disabled]), a[href], [role="separator"]')].filter((element) => element.getClientRects().length > 0);
...
async function applyCandidate(candidate: Candidate, action: "append" | "replace") {
  try {
    ...
    if (candidate.target === "storyboard") {
      activeTab.value = "scenes";
      await nextTick();
      sceneBoardElement.value?.scrollIntoView({ block: "start" });
    }
  } catch (error) {
...
async function deleteSelectedScene(sceneID = selectedSceneID.value) {
  const scene = draft.value.story.scenes.find((item) => item.id === sceneID);
  if (!scene || !window.confirm(`删除镜头 ${scene.order}“${scene.title}”？删除前会保留可恢复版本。`)) return;
  try {
    await requireSaved();
    acceptDraft(await api.deleteScene(workspace.projectId, draft.value.id, sceneID, draft.value.version || 0));
...
<template>
  <main class="studio-page creative-workspace">
    <div class="studio-project-bar">
      <header class="studio-head">
        <div class="studio-identity"><input v-model="draft.name" class="title-input" aria-label="草稿名称" /><p class="studio-meta"><span>{{ workspace.projectName || "新项目" }}</span><span aria-hidden="true">·</span><span class="save-indicator" :class="saveState">{{ saveState === "saved" ? "已保存" : saveState === "saving" ? "保存中" : saveState === "failed" ? "保存失败" : "尚未保存" }}</span></p></div>
        <div v-if="saveState === 'failed'" class="actions"><button @click="saveNow">重试保存</button></div>
      </header>
    ...
    <section v-if="activeTab === 'body'" class="workspace-body editor-card">
      <div class="section-head body-toolbar"><div class="body-toolbar-start"><span class="body-document-icon" aria-hidden="true">▤</span><h2>正文</h2><span class="body-toolbar-divider" aria-hidden="true"></span><button class="import-button" @click="importInput?.click()">导入文本</button></div><div class="actions body-toolbar-actions"><button class="story-assist-button" @click="openDrawer">故事联想</button><button class="primary generate-storyboard-button" :disabled="!canGenerateStoryboard" @click="generate('storyboard')">生成分镜</button></div></div>
      ...
      <div class="editor-footer"><span class="editor-hint">可直接输入或粘贴故事，也可以用故事联想开始创作。</span><span>{{ draft.story.body.length }} 字 · {{ saveState === "saved" ? "已保存" : saveState === "failed" ? "保存失败" : "自动保存" }} <span class="footer-divider" aria-hidden="true">|</span> 分镜 {{ draft.story.scenes.length }}</span></div>
      <div v-if="latestStoryboard" ref="storyboardCandidateElement" class="candidate-card"><h3>分镜候选 · {{ latestStoryboard.scenes?.length || 0 }} 个镜头</h3><p v-for="scene in latestStoryboard.scenes" :key="scene.order">{{ scene.order }}. {{ scene.title }} · {{ scene.duration_seconds }} 秒</p><button class="primary" @click="applyCandidate(latestStoryboard, 'replace')">{{ candidateNeedsReview === latestStoryboard.id ? "核对后确认替换" : "应用分镜" }}</button></div>
    </section>
    ...
    <section v-else-if="activeTab === 'scenes'" class="workspace-scenes">
      ...
      <div class="scene-layout" :class="{ 'has-selection': !!sceneForm }">
        <div ref="sceneBoardElement" class="scene-board">
          <div class="scene-board-header">
            <div class="scene-board-heading"><span class="scene-count">{{ draft.story.scenes.length }}</span><div><h2>分镜</h2><p>将故事拆解为镜头，逐一完善画面与素材</p></div></div>
            <div class="actions"><button @click="sceneAction('create')">＋ 新增分镜</button><button class="primary" :disabled="!draft.story.scenes.length || !videoCapability?.supported" @click="generateBatchVideo">批量生成视频</button></div>
          </div>
          <div class="scene-list" aria-label="分镜列表">
            <p v-if="!draft.story.scenes.length" class="empty">先从正文生成分镜，也可以手动新增。</p>
            <div v-for="scene in draft.story.scenes" :key="scene.id" class="scene-row-wrap">
              <button class="scene-row" :class="{ active: scene.id === selectedSceneID }" :aria-pressed="scene.id === selectedSceneID" @click="selectScene(scene.id)">
                <span class="scene-thumb"><img v-if="scenePreview(scene.id)" :src="scenePreview(scene.id)" alt="" loading="lazy" /><span v-else class="scene-thumb-empty">画面待添加</span><span class="scene-thumb-number">{{ String(scene.order).padStart(2, "0") }}</span></span>
                <span class="scene-row-copy"><strong>{{ scene.title || `镜头 ${scene.order}` }}</strong><span class="scene-description">{{ scene.visual_prompt || "暂未填写画面描述" }}</span><span class="scene-row-tags"><span>{{ (draft.bindings || []).filter((binding) => binding.scene_id === scene.id && !["driving_audio", "voiceover"].includes(binding.usage)).length }} 张参考图</span><span>{{ (draft.bindings || []).filter((binding) => binding.scene_id === scene.id && ["driving_audio", "voiceover"].includes(binding.usage)).length }} 段音频</span></span></span>
                <span class="scene-row-duration">{{ scene.duration_seconds }} 秒</span><span class="scene-status" :class="{ complete: sceneStatus(scene.id) === '已完成', running: ['生成中', '排队中'].includes(sceneStatus(scene.id)) }">{{ sceneStatus(scene.id) }}</span>
              </button>
              <button class="scene-row-delete" :aria-label="`删除镜头 ${scene.order}：${scene.title}`" @click="deleteSelectedScene(scene.id)">删除</button>
            </div>
          </div>
          <div class="scene-board-footer"><span>{{ draft.story.scenes.length }} 个镜头 · 预计总时长 {{ storyboardDuration }} 秒</span><button @click="activeTab = 'body'">返回正文</button></div>
        </div>
        <aside v-if="sceneForm" class="panel scene-inspector" aria-label="镜头设置">
          <div class="scene-inspector-heading"><button class="mobile-panel-back" @click="selectedSceneID = ''; sceneForm = null">返回镜头列表</button><h3>镜头 {{ String(selectedScene?.order || 0).padStart(2, "0") }}</h3><button class="scene-close" aria-label="关闭镜头设置" @click="selectedSceneID = ''; sceneForm = null">×</button></div>
          <div class="scene-inspector-content">
            <label class="scene-title-label">标题<input v-model="sceneForm.title" /></label>
            <label>画面描述<textarea v-model="sceneForm.visual_prompt" rows="4" /></label>
            <div class="scene-form-grid"><label>时长（秒）<input v-model.number="sceneForm.duration_seconds" type="number" min="1" /></label><label>画幅<input value="由视频模型决定" disabled /></label></div>
            <h3>参考图片</h3>
            <div v-for="usage in (['first_frame', 'last_frame', 'character_reference', 'scene_reference', 'prop_reference'] as SceneBinding['usage'][])" :key="usage" class="scene-material-row">
              <div class="scene-material-preview"><img v-if="materialForScene(selectedSceneID, usage)" :src="api.mediaUrl(materialForScene(selectedSceneID, usage)!.url)" alt="" /><span v-else>＋</span></div>
              <div class="scene-material-copy"><strong>{{ usageLabels[usage] }}</strong><small>{{ materialForScene(selectedSceneID, usage)?.name || "尚未选择素材" }}</small></div>
              <button @click="openPicker(usage)">{{ materialForScene(selectedSceneID, usage) ? "更换" : "选择" }}</button>
              <button v-if="materialForScene(selectedSceneID, usage)" :disabled="bindingBusy" @click="changeBinding(usage)">移除</button>
            </div>
            <h3>参考音频</h3>
            <div v-for="usage in (['driving_audio', 'voiceover'] as SceneBinding['usage'][])" :key="usage" class="scene-material-row">
              <div class="scene-material-preview audio">♫</div><div class="scene-material-copy"><strong>{{ usageLabels[usage] }}</strong><small>{{ materialForScene(selectedSceneID, usage)?.name || "尚未选择音频" }}</small></div>
              <button @click="openPicker(usage)">{{ materialForScene(selectedSceneID, usage) ? "更换" : "选择" }}</button><button v-if="materialForScene(selectedSceneID, usage)" :disabled="bindingBusy" @click="changeBinding(usage)">移除</button>
            </div>
            <label>文本旁白（可选）<textarea v-model="sceneForm.narration" rows="3" placeholder="填写旁白文本；不会自动合成语音" /></label>
          ...
          <div class="scene-edit-actions"><button @click="sceneAction('move', Math.max(1, (selectedScene?.order || 1) - 1))">上移</button><button @click="sceneAction('move', Math.min(draft.story.scenes.length, (selectedScene?.order || 1) + 1))">下移</button><button @click="sceneAction('copy')">复制</button><button class="danger-button" @click="deleteSelectedScene()">删除</button></div>
          </div>
          <div class="scene-inspector-footer"><button @click="sceneAction('update')">保存镜头</button><button class="primary" :disabled="!videoCapability?.supported" @click="generateSceneVideo">生成当前镜头视频</button></div>
        </aside>
      </div>
      <div v-if="draft.storyboard_snapshots?.length" class="history-strip"><h3>分镜历史</h3><button v-for="snapshot in draft.storyboard_snapshots" :key="snapshot.id" @click="restore(snapshot.id)">{{ snapshot.scenes?.length ?? 0 }} 个镜头 · 恢复</button></div>
    </section>
